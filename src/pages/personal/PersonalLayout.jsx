...
export default function PersonalLayout() {
  ...
  const pendingVideos = tenant
    ? getVideoReviewQueue(tenant.id).filter((v) => v.status === "pending")
        .length
    : 0;
  ...
  const SidebarContent = () => (
    <>
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-white font-black text-sm leading-none">
              {tenant?.appName || "Personal de Sucesso"}
            </p>
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {navItems.map((item) => {
          const showBadge =
            (item.to === "/personal/videos" && pendingVideos > 0) ||
            (item.to === "/personal/alunos" && alertCount > 0);
          const badgeCount =
            item.to === "/personal/videos" ? pendingVideos : alertCount;
          return (
            <NavLink
              key={item.to}
              to={item.to}
              ...
              onClick={() => setMenuOpen(false)}
              className={({ isActive }) =>
                `flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-bold transition-colors relative ${
                  isActive
                    ? "text-ink"
  ...
  return (
    <div className="min-h-screen bg-[#F3F3EE]">
      <div className="md:hidden sticky top-0 z-30 bg-ink flex items-center justify-between px-4 py-3">
