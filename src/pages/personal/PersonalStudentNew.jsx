  ...
  return (
    <div className="p-4 md:p-8 max-w-2xl">
      ...
      <div className="flex gap-1.5 mb-5">
        {steps.map((_, i) => (
          <div
            ...
            style={{ background: i <= step ? "#0B5A28" : "#E5E5E0" }}
          />
        ))}
      </div>

      <p className="text-xs font-bold uppercase tracking-wide text-forest mb-1">
        Novo aluno · Passo {step + 1}/3
      </p>
      <p className="font-display font-black text-2xl text-ink mb-6">
        {steps[step]}
      </p>
        ...
        <div className="space-y-4 mb-6">
          ...
          <div>
            ...
            <div className="grid grid-cols-3 gap-2">
              {[
                { v: "musc", l: "Musculação", i: Dumbbell },
                { v: "run", l: "Corrida", i: Activity },
                { v: "both", l: "Ambos", i: Zap },
              ].map((m) => (
                <button
                  key={m.v}
                  ...
                  style={{
                    borderColor: form.modality === m.v ? "#0B5A28" : "#E5E5E0",
                    background: form.modality === m.v ? "#E3F0E6" : "white",
                  }}
                >
                  <m.i
                    size={16}
                    ...
                    style={{
                      color: form.modality === m.v ? "#0B5A28" : "#8A8F8B",
                    }}
                  />
                  <p
                    className="text-[11px] font-bold"
                    style={{
                      color: form.modality === m.v ? "#0B5A28" : "#0D0F0E",
                    }}
                  >
                    {m.l}
                  </p>
      ...
      {step === 1 && (
        <div className="space-y-3 mb-6">
          {[
            {
              v: "hipertrofia",
              l: "Hipertrofia / ganho de massa",
              sub: "Musculação · foco em volume",
            },
            {
              v: "perda_peso",
              l: "Perda de peso",
              sub: "Misto · déficit + cardio",
            },
            {
              v: "primeira_corrida",
              l: "Primeira corrida de 10km",
              sub: "Corrida · base aeróbica",
            },
            {
              v: "performance",
              l: "Performance em prova específica",
              sub: "Corrida · periodização por data",
            },
          ].map((g) => (
            <button
              key={g.v}
              ...
              style={{
                borderColor: form.goal === g.v ? "#0B5A28" : "#E5E5E0",
                background: form.goal === g.v ? "#E3F0E6" : "white",
              }}
            >
              <div>
        ...
        <div className="space-y-4 mb-6">
          ...
          >
            ...
            <p className="text-xs leading-relaxed text-ink">
              Esses dados alimentam diretamente o gerador de periodização.
              Quanto mais completo, mais segura é a sugestão da IA.
            </p>
          </div>
          <TextField
            label="Lesões prévias"
            value={form.injuries}
            onChange={(v) => update("injuries", v)}
            placeholder="Ex: Tendinite no joelho direito (2023)"
          />
          <TextField
            label="Condições de saúde"
            value={form.healthConditions}
            onChange={(v) => update("healthConditions", v)}
            placeholder="Ex: Hipertensão controlada"
          />
          <TextField
            label="Histórico de treino"
            value={form.trainingHistory}
            onChange={(v) => update("trainingHistory", v)}
            placeholder="Ex: 2 anos de musculação"
          />
          <TextField
            label="Cirurgias / restrições médicas"
            value={form.surgeries}
            onChange={(v) => update("surgeries", v)}
            placeholder="Ex: Nenhuma"
          />
          <TextField
            label="Disponibilidade e equipamento"
            value={form.availability}
            onChange={(v) => update("availability", v)}
            placeholder="Ex: 4x/semana, academia completa"
          />
          <SelectField
            label="Tolerância à alta intensidade"
            value={form.intensityTolerance}
            onChange={(v) => update("intensityTolerance", v)}
            options={[
      ...
      <div className="flex gap-3">
        {step > 0 && (
          <OutlineButton onClick={() => setStep(step - 1)} className="flex-1">
            Voltar
          </OutlineButton>
        )}
        {step < 2 ? (
