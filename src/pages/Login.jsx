...
const demoAccounts = [
  {
    email: "admin@personaldesucesso.app",
    role: "admin",
  ...
  },
  {
    ...
    role: "personal",
    label: "Personal trainer (Bruno)",
    icon: Dumbbell,
  },
  {
    email: "ana@email.com",
    role: "aluno",
    label: "Aluna (Ana Beatriz)",
    icon: User,
  },
];
...
export default function Login() {
  ...
  const handleLogin = async (loginEmail) => {
    ...
    try {
      ...
      if (!user) {
        setError(
          "E-mail não encontrado. Use uma das contas de demonstração abaixo."
        );
        setLoading(false);
        return;
      }
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F3F3EE] px-4">
      <div className="w-full max-w-sm">
        <div className="flex flex-col items-center mb-8">
          <Logo size={48} />
          <p className="font-display font-black text-2xl text-ink mt-3">
            Personal de Sucesso
          </p>
        ...
        <div className="bg-white rounded-2xl border border-black/6 p-5 mb-5">
          <label className="text-xs font-bold text-stone mb-1.5 block">
            E-mail
          </label>
          <input
            value={email}
          ...
          <button
            onClick={() => handleLogin()}
            disabled={loading}
            className="w-full bg-ink text-white rounded-xl py-3.5 font-bold text-sm hover:opacity-90 flex items-center justify-center gap-2 disabled:opacity-60"
          >
            {loading ? (
              <>
                <Spinner size={16} color="white" /> Entrando...
              </>
            ) : (
              "Entrar"
            )}
        ...
        <p className="text-[11px] font-bold uppercase tracking-wide text-stone mb-2.5 text-center">
        ...
        <div className="space-y-2">
          {demoAccounts.map((acc) => (
            <button
              ...
              disabled={loading}
              className="w-full bg-white rounded-xl border border-black/6 p-3.5 flex items-center gap-3 text-left hover:border-forest/40 transition-colors disabled:opacity-60"
            >
