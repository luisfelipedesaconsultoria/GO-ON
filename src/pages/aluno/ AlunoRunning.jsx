...
import { Footprints, Target, Heart, Play } from "lucide-react";
...
export default function AlunoRunning() {
  ...
  if (student.modality === "musc") {
    return (
      <div className="px-5 pt-10 text-center">
        <p className="text-white/60 text-sm">Seu plano atual é focado em musculação. Fale com seu personal se quiser incluir corrida.</p>
      </div>
    );
  }
  ...
  return (
    <div className="px-5 pt-4">
      <div className="flex items-center justify-between mb-5">
        <p className="text-white font-black text-xl">Corrida</p>
        <span className="text-[10px] font-bold px-2.5 py-1 rounded-full" style={{ background: "rgba(200,245,98,0.18)", color: "#C8F562" }}>
          {student.phase}
        </span>
      </div>

      <div className="rounded-3xl p-5 mb-5 bg-charcoal border border-white/8">
        ...
        <div className="grid grid-cols-3 gap-2 mb-4">
          <div className="rounded-xl p-3 bg-white/[0.06]">
        ...
        <button className="w-full rounded-xl py-3.5 font-bold text-sm flex items-center justify-center gap-2 bg-lime text-ink">
          <Play size={15} fill="#0D0F0E" /> Iniciar corrida
        </button>
      </div>

      <p className="text-xs font-bold uppercase tracking-wide text-white/45 mb-2.5">Últimas corridas</p>
      <div className="space-y-2">
        {[
          { d: "Segunda", t: "Rodagem leve", dist: "8.0km", pace: "5:40" },
          { d: "Sexta passada", t: "Longão", dist: "12.0km", pace: "5:25" },
        ].map((r, i) => (
          <div key={i} className="rounded-xl p-3.5 flex items-center justify-between bg-white/[0.06]">
            <div>
              <p className="text-white font-bold text-sm">{r.t}</p>
              <p className="text-[11px] text-white/45">{r.d}</p>
            </div>
            <div className="text-right">
              <p className="text-white font-mono text-sm">{r.dist}</p>
              <p className="text-[11px] font-mono text-white/45">{r.pace}/km</p>
            </div>
          </div>
        ))}
      </div>
